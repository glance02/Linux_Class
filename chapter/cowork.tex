...
这些操作在代码中的定义集中在 \cmd{code/cowork/document.py}：\cmd{VALID\_KINDS} 限定操作类型只能是 \cmd{insert}、\cmd{delete} 和 \cmd{noop}，\cmd{Operation} 数据类保存 \cmd{kind}、\cmd{pos}、\cmd{char} 和可选的 \cmd{reason}。客户端在 \cmd{code/cowork/client.py} 的按键处理函数中把普通字符、回车和 Backspace 转换成 \cmd{Operation}，再由 \cmd{code/cowork/client\_state.py} 封装成网络层的 \cmd{OP} 消息。服务端终端上看到的 \cmd{OP v...} 并不是另一种新的操作模型，而是 \cmd{code/cowork/server.py} 中 \cmd{format\_operation\_event} 对服务端日志事件的可读化输出。
...
\inputminted[firstline=5,lastline=65,firstnumber=5]{python}{code/cowork/protocol.py}

\subsubsection{编辑操作模型}

编辑操作被抽象为 \cmd{Operation}，支持 \cmd{insert}、\cmd{delete} 和 \cmd{noop}。从网络消息解析操作时会进行类型检查，例如插入操作必须携带单个字符。

\inputminted[firstline=5,lastline=54,firstnumber=5]{python}{code/cowork/document.py}

\subsubsection{服务端 OP 日志输出}

服务端终端中的 \cmd{OP v...} 输出由 \cmd{format\_operation\_event} 生成。它从操作日志事件中取出服务端版本、客户端编号、变换后的操作、字符内容和基准版本，并格式化成便于演示观察的一行文本。

\inputminted[firstline=20,lastline=31,firstnumber=20]{python}{code/cowork/server.py}

\subsubsection{OT 位置变换规则}

\cmd{transform\_against} 是 OT 核心函数。它根据一条已经发生的历史操作，调整当前操作的位置，或者在重复删除同一字符时将操作转换为 \cmd{noop}。

\inputminted[firstline=102,lastline=131,firstnumber=102]{python}{code/cowork/document.py}
...
\inputminted[firstline=163,lastline=287,firstnumber=163]{python}{code/cowork/document.py}

\subsubsection{服务端锁外发送与广播}

服务端处理操作时，文档修改已经在 \cmd{process\_operation} 内完成。返回结果后，服务端再发送 ACK 和广播远程操作，从而避免慢客户端阻塞文档锁。

\inputminted[firstline=209,lastline=284,firstnumber=209]{python}{code/cowork/server.py}
...
\inputminted[firstline=13,lastline=120,firstnumber=13]{python}{code/cowork/client_state.py}

\subsubsection{终端按键处理}

终端客户端将用户按键映射为协作编辑操作。普通字符、回车和 Backspace 分别转化为插入字符、插入换行和删除光标前字符；\cmd{Ctrl-S} 和 \cmd{Ctrl-Q} 分别用于保存和退出。

\inputminted[firstline=192,lastline=240,firstnumber=192]{python}{code/cowork/client.py}

\subsubsection{自动保存与备份}

自动保存进程通过队列接收主服务端发来的事件。它负责定期写入自动保存文件，手动保存时生成备份并写入正式文件，同时追加 JSON 日志。

\inputminted[firstline=15,lastline=103,firstnumber=15]{python}{code/cowork/autosave.py}

\subsubsection{文档一致性测试}

文档测试覆盖了 OT 中最关键的情况：旧版本删除位置经过多个插入后右移、重复删除变为 \cmd{noop}、历史过期触发重同步，以及版本号严格递增。

\inputminted[firstline=6,lastline=100,firstnumber=6]{python}{code/cowork/tests/test_document.py}

\subsubsection{客户端队列测试}

客户端状态测试验证了 ACK 驱动发送队列、远程版本缺口处理、服务端分配客户端编号、中文字符输入和宽字符删除等行为。

\inputminted[firstline=8,lastline=96,firstnumber=8]{python}{code/cowork/tests/test_client_state.py}
