...
本项目名为 TermSync，是一个使用 C 语言和 POSIX 接口实现的终端多人协作代码编辑器。项目的目标不是复刻完整的图形化 IDE，而是在 Linux/WSL 环境下实现一个可实际运行、可多人连接、可同步编辑、可保存和可测试验证的轻量级协作编辑系统。用户在终端中运行客户端，通过 TCP 连接到服务端；服务端维护唯一权威文档状态，并负责对所有编辑操作进行排序、变换、应用和广播。

我刻意减少第三方依赖，主要使用 C 标准库、POSIX 系统调用和少量 Linux 常用开发库完成系统实现。网络通信基于 POSIX \cmd{socket}，并发处理使用 \cmd{pthread}，终端界面使用 \cmd{ncursesw}，消息格式和持久化则由项目内的轻量 JSON Lines 编码、解析和文件写入函数完成。服务端使用 \cmd{fork()} 创建自动保存进程，并通过 pipe 向保存进程投递快照、保存和日志事件。项目没有依赖现成的 Web 框架、协同编辑库或数据库系统，因此网络协议、客户端状态机、服务端 OT 变换、广播同步和保存机制都由项目代码自行实现，更能体现 Linux 程序设计中对进程、线程、文件和网络等基础能力的直接使用。

本项目覆盖了 Linux 程序设计中的多个核心主题：网络编程、线程创建与同步、进程间通信、文件系统操作、终端界面编程和自动化测试。服务端使用 socket 监听客户端连接，为每个客户端创建独立线程；文档对象使用互斥锁保护共享状态；自动保存和日志写入被拆分到独立进程中，通过 pipe 与主服务端通信；客户端使用 \cmd{ncursesw} 构建终端交互界面。
...
在实际部署中，服务端运行在我的个人 Linux 服务器上。这台服务器是拥有独立公网 IP 的阿里云 ECS 服务器，并已经绑定域名 \cmd{glance02.xyz}，因此客户端后续不需要直接记忆服务器公网 IP，而是通过域名连接服务端。例如服务端在 Linux 服务器上监听 \cmd{0.0.0.0:8765}，客户端可以使用 \cmd{glance02.xyz:8765} 加入同一个协作文档。由于我的两台电脑都是 Windows 系统，客户端实际运行在 Windows 的 WSL（Windows Subsystem for Linux）环境中。WSL 提供了较完整的 Linux 用户空间环境，支持 GCC、POSIX socket、pthread、ncursesw 和 TCP 网络编程，因此客户端可以直接在 WSL 终端中编译运行，并通过公网域名连接到服务器。

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{pic/cowork/服务端运行界面.png}
    \caption{服务端运行界面}
    \label{fig:cowork-server-running}
\end{figure}

图~\ref{fig:cowork-server-running} 展示了服务端运行界面。服务端已经在 Linux 服务器上启动，并监听指定端口等待客户端接入；终端输出中可以看到监听地址、端口和后续客户端连接产生的运行信息。
...
\begin{figure}[H]
    ...
    \caption{两个客户端连接服务端}
    \label{fig:cowork-clients-connect}
\end{figure}



图~\ref{fig:cowork-clients-connect} 展示了两个客户端连接服务端后的界面。两个客户端通过域名连接到同一个远程服务端，并分别以不同的客户端身份进入终端编辑界面，说明新加入的客户端能够获得服务端发送的当前共享文档状态，也为后续观察多人同步编辑、远程广播和保存验证提供了运行基础。

图~\ref{fig:cowork-server-running} 和图~\ref{fig:cowork-clients-connect} 合起来说明，TermSync 的服务端可以持续监听公网连接，并同时维护多个客户端会话；新加入的客户端能够进入同一个协作文档，而不是各自打开孤立的本地文本。
...
本项目也具有明确的 Linux 课程背景。首先，服务端是一个典型的 TCP 长连接服务，涉及 socket 创建、地址绑定、监听、接受连接和消息收发。其次，每个客户端连接由独立线程处理，多个线程会同时访问共享文档，因此必须使用锁保护临界区。再次，自动保存和日志写入使用独立进程完成，主进程通过 pipe 向保存进程发送快照、保存和日志事件，体现了进程间通信的使用。最后，文件加载、原子写入、备份文件、自动保存文件和编辑日志都体现了 Linux 文件系统编程的基本实践。

此外，终端编辑器本身也与 Linux 使用场景贴合。客户端基于 \cmd{ncursesw} 运行在终端中，支持方向键移动、普通字符输入、回车换行、Backspace 删除、\cmd{Ctrl-S} 保存和 \cmd{Ctrl-Q} 退出。相比网页前端，这种实现更接近 Linux 命令行环境下的工具开发，也便于在服务器和 WSL 中演示。
...
这些操作在代码中的定义集中在 \cmd{termsync.h} 和 \cmd{document.c} 中：\cmd{TsOpKind} 限定操作类型只能是 \cmd{insert}、\cmd{delete} 和 \cmd{noop}，\cmd{TsOperation} 结构体保存 \cmd{kind}、\cmd{pos}、\cmd{ch} 和可选的 \cmd{reason}。客户端在 \cmd{client.c} 的按键处理函数中把普通字符、回车和 Backspace 转换成 \cmd{TsOperation}，再由 \cmd{client\_state.c} 封装成网络层的 \cmd{OP} 消息。服务端终端上看到的 \cmd{OP v...} 并不是另一种新的操作模型，而是 \cmd{events.c} 中日志格式化函数对服务端事件的可读化输出。
...
字符可以输入英文、数字、符号，也可以输入中文等宽字符。客户端使用 C 的 UTF-8 与宽字符辅助函数，通过 \cmd{mbrtowc()}、\cmd{wcwidth()} 等能力计算字符显示宽度，并提供显示列和字符列之间的转换函数，确保光标移动和文本显示在终端中正确对齐。
...
图~\ref{fig:log and bak}中展示了服务端运行目录中的保存产物和编辑日志。左侧可以看到正式文档 \cmd{shared.c}、自动保存文件 \cmd{shared.c.autosave}、备份文件 \cmd{shared.c.bak} 以及编辑日志 \cmd{shared.c.edit.log} 同时存在，说明服务端把文档内容、自动保存快照、覆盖前备份和事件记录分开管理。右侧打开的日志文件由多条 JSON 事件组成，每条记录包含 \cmd{event}、\cmd{path}、\cmd{timestamp} 和 \cmd{version} 等字段；其中 \cmd{autosave} 事件表示自动保存进程已经按固定间隔将当前快照写入 \cmd{.autosave} 文件。后续发生手动保存或编辑操作时，同一日志文件还会继续追加 \cmd{manual\_save}、\cmd{operation} 等记录，用于追踪服务端文档状态的变化过程。
...
\begin{itemize}
  \item \cmd{protocol.c}：封装 JSON Lines 消息的编码、解码和 socket 收发。
  \item \cmd{document.c}：实现权威文档、操作模型、OT 变换、版本历史和重同步判断。
  \item \cmd{server.c}：实现 TCP 服务端、客户端线程、消息分发、广播和自动保存进程管理。
  \item \cmd{client\_state.c}：实现客户端同步状态、待发送队列、ACK 处理和远程操作应用。
  \item \cmd{client.c}：实现 ncursesw 终端界面、光标移动、按键处理和网络连接。
  \item \cmd{autosave.c}：实现自动保存、手动保存、备份和日志写入工作进程。
  \item \cmd{tests/test\_termsync.c}：包含文档、客户端状态、协议、服务端输出和自动保存相关测试。
\end{itemize}

为了更直观地说明各个模块之间的关系，我将系统划分为界面层、网络协议层、同步控制层、权威文档层和持久化层。客户端侧主要覆盖界面层、发送队列和接收消息处理；服务端侧则集中承担权威文档、全局顺序、操作变换、广播和落盘。这样的分层使每一部分都能对应到一个比较明确的 Linux 程序设计主题：终端界面对应 \cmd{ncursesw}，网络协议对应 \cmd{socket}，并发处理对应 \cmd{pthread}，自动保存对应 \cmd{fork()}、\cmd{pipe} 和文件系统操作。
...
自动保存和日志写入被放在独立进程中完成。主服务端进程处理网络和文档同步，自动保存进程负责磁盘 I/O。二者通过 \cmd{pipe} 传递事件。服务端每处理完一次成功编辑，就发送最新快照；工作进程只保留最新内容，并按固定间隔写入 \cmd{.autosave} 文件。

手动保存时，服务端通过 pipe 发送 \cmd{SAVE\_NOW} 事件。保存进程先复制旧文件作为 \cmd{.bak} 备份，再使用临时文件替换的方式写入正式文件。这样既体现了进程间通信，也减少了主服务端在网络请求处理过程中直接进行磁盘写入的时间。
...
自动保存流程的设计重点在于把“编辑同步”和“磁盘写入”分开。服务端主线程和客户端处理线程负责响应网络事件，尽量不要因为磁盘 I/O 而阻塞；保存进程则通过 pipe 接收最新快照，按照固定间隔写入 \cmd{.autosave} 文件。手动保存时，保存进程会先备份旧文件，再写入正式文件，这样即使用户在演示时误保存，也仍然可以从 \cmd{.bak} 中恢复上一个正式版本。
...
项目使用 C 语言和 POSIX 接口 \cmd{C 测试 runner} 进行自动化测试。
...
测试验证并不是只为了得到一个通过提示，更重要的是把项目中最容易出错的状态变化固定下来。TermSync 的风险点主要集中在三个方面：第一，多线程同时修改文档时版本号是否仍然严格递增；第二，旧版本操作经过历史变换后是否仍然表达原来的编辑意图；第三，客户端连续输入、远程广播和重同步之间是否会造成队列状态错乱。因此测试用例围绕 OT 变换、版本控制、客户端队列、协议编解码、保存机制和服务端事件输出展开。

实际运行时，可以在 Linux 服务器克隆 \cmd{code} 分支后，进入克隆得到的代码仓库根目录执行测试命令：
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{pic/cowork/test1.png}
    \caption{\cmd{make test} 单元测试通过结果}
\end{figure}

从图中的实际输出可以看到，运行 \cmd{make test} 后，构建系统会先完成项目模块和测试文件的编译，再链接生成并运行测试程序 \cmd{./build/test\_termsync}。测试程序按 \cmd{[TEST]} 和 \cmd{[PASS]} 成对输出验证项。

本次截图共出现 9 组单元测试，最后以 \cmd{All TermSync C tests passed.} 结束，说明当前 C 端核心逻辑全部通过验证。

测试覆盖的关键场景包括：

\begin{itemize}
  \item \cmd{OT 删除位置转换}：旧版本删除操作经过之后的插入操作变换后，删除位置能够正确右移。
  \item \cmd{OT 重复删除冲突}：两个客户端同时删除同一字符时，后到达的删除会被转换为 \cmd{NOOP}。
  \item \cmd{历史过期重同步}：历史记录被裁剪后，过旧 \cmd{base\_version} 的操作会触发 \cmd{RESYNC\_REQUIRED}。
  \item \cmd{版本递增与 UTF-8}：服务端版本号严格递增，并确认中文字符不会被按字节拆开。
  \item \cmd{客户端 ACK 节奏}：客户端同一时间只发送一个 \cmd{inflight} 操作，收到 ACK 后才发送下一条。
  \item \cmd{远程版本缺口修复}：客户端发现远程版本跳跃时，会请求完整文档快照恢复状态。
  \item \cmd{JSON Lines 协议}：协议编解码能够处理换行和 UTF-8 文档内容。
  \item \cmd{备份与自动保存}：正式保存生成 \cmd{.bak}，自动保存子进程写入 \cmd{.autosave}。
  \item \cmd{服务端事件格式化}：JSON 日志可以被格式化成人类可读的终端事件。
\end{itemize}

这些测试大多不依赖真实网络连接，而是直接验证核心状态变化。这样做可以使测试运行更快，也能让错误定位更集中。例如文档一致性问题可以直接在 \cmd{document.c} 的测试中暴露，不需要同时启动两个客户端和一个服务端才能复现；协议转义、自动保存文件和服务端事件格式化也可以在本地测试进程中直接检查。真实网络验证仍然是必要的，但它更适合放在后面的本地验证和部署验证中，用来确认多个模块组合后仍然能够正常工作。
...
\begin{minted}{bash}
./build/termsync_server --host 127.0.0.1 --port 8765 --file shared.c
\end{minted}
...
\begin{minted}{bash}
./build/termsync_client --host 127.0.0.1 --port 8765 --client-id sputnik
./build/termsync_client --host 127.0.0.1 --port 8765 --client-id glance
\end{minted}
...
图中将原始 JSON 日志中的 \cmd{transform\_trace} 提取为位置变换过程。该操作的 \cmd{op\_id} 是 \cmd{stale-transform}，提交时基于旧版本 0，并在服务端版本 5 被确认。操作原本希望在位置 120 插入字符，服务端根据之后已经发生的历史插入操作依次调整位置，最终将其应用到位置 124。（图中编辑的文件是 \cmd{shared.py}，前面有些图里面是 \cmd{shared.c}，这个只需要在启动服务器的时候修改一下 --file 参数就可以）
...
项目实现过程中遇到的问题并不是互相独立的。例如“连续输入容易乱序”与“旧版本操作位置失效”都和客户端本地版本落后有关；“慢客户端阻塞服务端”和“文件保存耗时”都和临界区范围有关；“服务端退出不可靠”涉及信号处理、阻塞系统调用和保存进程的生命周期；“中文字符显示宽度”虽然看起来只是界面问题，但如果光标位置换算错误，最终也会影响用户提交的编辑位置。因此本节按照问题来源整理解决方案，并说明这些方案在代码中的体现。
...
\subsubsection{中文连续输入时光标被 ACK 拉回}

问题：客户端采用服务端权威模型，本地输入会先进入 \cmd{pending\_queue} 或 \cmd{inflight\_op}，只有收到服务端 ACK 后才真正更新 \cmd{state.content}。如果用户连续快速输入多个中文字符，ACK 返回时客户端若只用“已确认文本”的长度夹紧光标，就会把光标错误拉回旧位置。后续中文字符虽然已经排队，但生成操作时使用的插入位置会偏小，表现为前两个字符位置正常，后面的字符位置开始重复或错位。

解决方案：光标夹紧时不能只参考 \cmd{state.content}，而要参考预计文本长度。这个预计文本由“已确认文本 + 当前 \cmd{inflight\_op} + \cmd{pending\_queue} 中尚未发送的操作”共同决定。这样 ACK 到达后，即使服务端只确认了前面一部分输入，客户端也不会把用户当前光标拉回到已经确认文本的末尾，连续中文输入生成的位置就能保持递增。

这个问题说明 ACK 队列不仅影响网络发送顺序，也影响终端交互状态。英文快速输入时问题不一定明显，因为字符宽度和显示位置较简单；中文输入既包含多字节 UTF-8，又通常占两个终端显示列，一旦光标位置被旧内容长度夹紧，就会同时造成协议层位置错误和界面显示偏移。因此客户端需要同时维护“服务端已确认状态”和“用户正在输入的预计状态”。
...
\subsubsection{服务端 Ctrl-C 退出不可靠}

问题：服务端主线程会阻塞在 \cmd{accept()} 等待新客户端连接。原来信号处理函数只是把退出标志置位，但在 Linux 上阻塞系统调用可能被 \cmd{signal()} 安装的处理方式自动重启，主循环没有机会及时重新检查退出标志。因此真实终端中按下 \cmd{Ctrl-C} 时，服务端可能仍停在 \cmd{accept()} 中，看起来无法正常退出。

另一个相关问题是自动保存子进程的生命周期。终端发送 \cmd{Ctrl-C} 时，\cmd{SIGINT} 会发给整个前台进程组；如果自动保存子进程在父进程安装信号处理之前已经 fork 出来，它可能按默认行为直接退出。这样父进程进入 shutdown 流程时，最后一次保存事件就可能无法送达保存进程，影响最终文档落盘。

解决方案：服务端应使用 \cmd{sigaction()} 安装 \cmd{SIGINT} 和 \cmd{SIGTERM} 处理函数，并避免依赖可能自动重启的阻塞 \cmd{accept()}。监听 socket 可以设置为非阻塞，主循环使用带短超时的 \cmd{select()} 等待连接事件；即使没有新客户端到来，循环也能周期性返回并检查退出标志。信号处理应在启动自动保存进程之前完成，使子进程继承合适的信号行为。同时忽略 \cmd{SIGPIPE}，避免客户端 socket 或保存 pipe 断开时把服务端直接终止。

这个问题体现了 Linux 服务程序关闭流程的细节：优雅退出不是只捕获一个信号，还要保证主循环能从阻塞等待中醒来，并保证子进程、管道和最终保存按照正确顺序收尾。对于 TermSync 来说，\cmd{Ctrl-C} 退出时还应投递最终保存事件，再关闭 autosave pipe 并等待保存进程结束，才能尽量保证共享文档、自动保存文件和日志处于一致状态。

\subsubsection{中文字符显示宽度}

问题：终端中中文字符通常占两个显示列，如果简单按照字符串长度或 UTF-8 字符个数移动光标，显示位置会错乱。同时，中文字符在输入法提交时会以多字节 UTF-8 序列进入终端，如果读取循环完全非阻塞，过于频繁轮询也可能增加输入被拆分处理的风险。

解决方案：客户端使用 C 的 UTF-8 与宽字符辅助函数，通过 \cmd{mbrtowc()}、\cmd{wcwidth()} 等能力计算宽字符显示列，并提供显示列和字符列之间的转换函数。终端输入读取也可以从完全非阻塞改为短超时，例如让 \cmd{get\_wch()} 等待约 50ms，以降低中文多字节输入在终端层被拆坏的概率。测试中也包含中文字符插入、删除和长度计算场景。
...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.20\textwidth} p{0.28\textwidth} p{0.36\textwidth}}
        ...
        课程知识点 & 项目中的体现 & 实际作用 \\
        \midrule
        TCP 网络编程 & \cmd{server.c} 监听端口，\cmd{client.c} 主动连接 & 让多个终端客户端能够加入同一个协作文档 \\
        多线程 & 服务端为每个客户端创建处理线程 & 同时接收多个客户端消息，避免一个客户端独占服务端 \\
        线程同步 & \cmd{CollaborativeDocument} 内部使用锁 & 保证文本、版本号和历史记录的一致性 \\
        进程间通信 & 主服务端通过 \cmd{pipe} 与保存进程通信 & 把磁盘写入从网络处理流程中拆出去 \\
        文件系统 & 正式文档、自动保存、备份、日志和临时文件 & 实现持久化、恢复和运行记录追踪 \\
        终端界面 & \cmd{ncursesw} 绘制编辑区域和状态栏 & 在 Linux 命令行中提供交互式编辑体验 \\
        自动化测试 & \cmd{C 测试 runner} 覆盖文档、客户端状态和保存逻辑 & 用可重复测试验证同步算法和边界情况 \\
        \bottomrule
    \end{tabular}
\end{table}
...
服务端主进程负责网络同步，保存进程负责磁盘写入，这种拆分体现了进程间通信的使用。主进程处理完编辑操作后，不直接写文件，而是把快照和事件写入 pipe。保存进程从 pipe 中读取事件，根据事件类型决定是更新自动保存内容、立即写入正式文件，还是追加日志。
...
与普通命令行程序不同，\cmd{ncursesw} 程序会接管终端屏幕，持续刷新显示内容。这要求客户端把“文档内部位置”和“屏幕显示位置”区分开。文档内部位置是字符串索引，协议和服务端都只关心这个索引；屏幕显示位置则要考虑换行、终端宽度和中文字符占用两列等问题。项目中专门实现字符宽度辅助函数，就是为了解决这个差异。
...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.22\textwidth} p{0.32\textwidth} p{0.30\textwidth}}
        ...
        \midrule
        单元测试 & 文档变换、客户端队列、保存备份等确定性逻辑 & \cmd{C 测试 runner} 运行结果和测试覆盖表 \\
...
\subsubsection{关键运行流程概览}

前面的设计说明从模块角度解释系统结构，下面用一张表概括几个关键运行场景，便于和后面的代码展示对应起来。
...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.18\textwidth} p{0.30\textwidth} p{0.36\textwidth}}
        ...
        流程 & 主要参与模块 & 核心结果 \\
        \midrule
        客户端加入 & \cmd{client.c}、\cmd{server.c}、\cmd{document.c} & 建立 TCP 连接，返回 \cmd{WELCOME} 和完整 \cmd{DOC\_STATE} \\
        编辑确认 & \cmd{client\_state.c}、\cmd{document.c}、\cmd{server.c} & 操作进入队列，服务端确认版本，返回 ACK 并广播远程操作 \\
        保存与关闭 & \cmd{server.c}、\cmd{autosave.c}、文件系统 & 投递快照和保存事件，生成正式文件、备份、自动保存和日志 \\
        重同步恢复 & \cmd{document.c}、\cmd{client\_state.c}、\cmd{protocol.c} & 版本缺口或历史过期时回到服务端权威快照 \\
        \bottomrule
    \end{tabular}
\end{table}
...
本文所对应的项目代码托管于 GitHub 仓库 \url{https://github.com/glance02/Linux_Class.git} 中。其中，程序实现位于 \cmd{code} 分支，论文正文的 LaTeX 源码位于 \cmd{master} 分支。C 版本完整实现包含服务端、客户端、协议封装、持久化、日志和测试等多个模块；本节尽量展示主要源码，使前文提到的同步、保存和测试逻辑都能在代码中直接对应。下文代码展示中涉及的文件路径，均以服务器克隆 \cmd{code} 分支后得到的代码仓库根目录为基准。

\subsubsection{项目结构}

TermSync 的 C 项目实现代码在 \cmd{code} 分支中位于仓库根目录。下面的项目结构按照服务器克隆 \cmd{code} 分支后的根目录书写：

\begin{minted}[fontsize=\small,breaklines]{text}
Linux_Class/                # code 分支克隆后的仓库根目录
|-- Makefile                # 构建服务端、客户端和测试程序
|-- README.md               # 项目运行方式与功能说明
|-- plan.md                 # C 版项目设计与实现计划记录
|-- termsync.h              # 公共类型、消息结构和函数声明
|-- util.c                  # UTF-8 字符位置、文件读取和通用工具
|-- protocol.c              # JSON Lines 编码、解析和 socket 收发封装
|-- document.c              # 权威文档模型、OT 变换、版本历史和重同步
|-- client_state.c          # 客户端 ACK 队列、版本同步和远程操作处理
|-- autosave.c              # fork 保存进程、pipe 事件、备份和日志写入
|-- events.c                # 服务端 JOIN、OP、SAVE、LEAVE 事件格式化
|-- server.c                # TCP 多线程服务端、广播和保存触发
|-- client.c                # ncursesw 终端客户端与键盘交互逻辑
`-- tests/
    `-- test_termsync.c     # 文档、协议、客户端状态和保存逻辑测试
\end{minted}

其中，\cmd{server.c}、\cmd{document.c}、\cmd{autosave.c} 构成服务端核心；\cmd{client.c} 和 \cmd{client\_state.c} 构成终端客户端；\cmd{protocol.c} 被客户端和服务端共同使用，用于保证两端消息格式一致。\cmd{tests/test\_termsync.c} 使用简单断言函数组织测试，覆盖同步算法、客户端状态、协议转义、保存备份和服务端输出等关键行为。

\subsubsection{构建规则与公共类型}

\textbf{构建规则：}\cmd{Makefile} 定义了三个主要产物：\cmd{termsync\_server}、\cmd{termsync\_client} 和 \cmd{test\_termsync}。默认执行 \cmd{make} 会构建三者；如果云服务器只负责运行服务端，可以先执行 \cmd{make server}，避免因为未安装终端客户端所需的 \cmd{ncursesw} 开发包而阻塞部署。公共模块会被同时链接到服务端、客户端和测试程序，客户端额外链接 \cmd{ncursesw}。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{pic/cowork/make项目.png}
    \caption{运行 \cmd{make} 生成可执行程序}
    \label{fig:cowork-make-build}
\end{figure}

图~\ref{fig:cowork-make-build} 展示了在项目根目录执行 \cmd{make} 后的构建结果。编译过程会先生成各个模块的目标文件，再链接得到服务端、客户端和测试程序等可执行文件，说明项目可以通过统一的构建规则完成完整编译。

\inputminted[firstline=1,lastline=63,firstnumber=1]{makefile}{code/cowork/Makefile}

\textbf{公共类型：}\cmd{termsync.h} 定义协议消息、编辑操作、传输层、文档状态、处理结果、客户端状态和保存进程等结构。C 语言没有动态字典和数据类，因此消息字段、字符串长度、内存所有权和释放规则都需要在公共头文件中明确约定。

\inputminted[firstline=1,lastline=228,firstnumber=1]{c}{code/cowork/termsync.h}

\subsubsection{协议与 UTF-8 文本工具}

\textbf{协议封装：}协作编辑首先需要稳定的消息边界和统一的数据表示。项目使用 JSON Lines 作为 TCP 字节流上的应用层协议：发送时把 \cmd{TsMessage} 编码为 UTF-8 JSON，并在末尾追加换行符；接收时先在缓冲区中寻找换行符，再解析出完整消息。协议封装还为发送操作加锁，避免同一连接被多个线程同时写入时出现字节交叉。

\inputminted[firstline=1,lastline=566,firstnumber=1]{c}{code/cowork/protocol.c}

\textbf{UTF-8 文本工具：}C 字符串本质上是字节数组，而协作编辑需要按照“字符位置”处理插入和删除。\cmd{util.c} 中的辅助函数负责统计 UTF-8 字符数、把字符位置换算为字节偏移，并按字符边界应用 \cmd{insert} 和 \cmd{delete}，从而保证中文字符不会被拆开。

\inputminted[firstline=1,lastline=260,firstnumber=1]{c}{code/cowork/util.c}

\subsubsection{服务端权威文档与 OT}

\textbf{操作模型与位置变换：}\cmd{document.c} 中的 \cmd{TsOperation} 支持 \cmd{insert}、\cmd{delete} 和 \cmd{noop}。\cmd{ts\_transform\_against} 根据一条已经发生的历史操作调整当前操作的位置；如果两个删除操作指向同一字符，后到达的删除会变为 \cmd{noop}，避免误删后续字符。

\inputminted[firstline=1,lastline=322,firstnumber=1]{c}{code/cowork/document.c}

\textbf{权威文档处理流程：}\cmd{ts\_document\_process\_operation} 展示了服务端主导 OT 的完整流程：检查基准版本、遍历历史、执行变换、规范化操作、分配服务端版本号、应用文本修改、写入历史，并生成 ACK、远程广播、保存快照和日志事件。

\subsubsection{服务端连接、广播与事件输出}

\textbf{事件输出：}\cmd{events.c} 把结构化日志事件转换成便于课堂演示观察的终端输出，例如 \cmd{JOIN client=...}、\cmd{OP v...}、\cmd{SAVE requested...} 和 \cmd{LEAVE client=...}。这些格式化函数不改变网络协议，只服务于调试和报告展示。

\inputminted[firstline=1,lastline=95,firstnumber=1]{c}{code/cowork/events.c}

\textbf{服务端主体：}\cmd{server.c} 在启动时读取协作文档、初始化权威文档对象、创建保存进程并监听 TCP 端口。每个客户端连接由一个线程处理；编辑操作进入文档模型，保存请求投递给保存进程，文档请求则直接返回最新快照。操作处理完成后，服务端在文档锁外发送 ACK 和广播，避免慢客户端阻塞文档临界区。

\inputminted[firstline=1,lastline=482,firstnumber=1]{c}{code/cowork/server.c}

\subsubsection{客户端状态机与终端界面}

\textbf{客户端同步状态：}\cmd{client\_state.c} 的关键是 \cmd{pending\_queue} 和 \cmd{inflight\_op}。用户连续输入产生的操作先进入队列；同一时刻客户端最多发送一个未确认操作，收到服务端 ACK 后再发送下一个。客户端应用的始终是服务端返回的权威操作，因此不需要在本地实现复杂 OT。

\inputminted[firstline=1,lastline=212,firstnumber=1]{c}{code/cowork/client_state.c}

\textbf{ncursesw 客户端主体：}\cmd{client.c} 把网络接收和终端界面分开：后台线程只负责从 socket 读取消息并放入队列，界面线程负责消费消息、渲染状态栏和正文，并把用户按键转换为 \cmd{TsOperation}。普通字符、回车和 Backspace 会变成协作编辑操作，\cmd{Ctrl-S} 和 \cmd{Ctrl-Q} 分别用于保存和退出。

\inputminted[firstline=1,lastline=490,firstnumber=1]{c}{code/cowork/client.c}

\subsubsection{自动保存、备份与日志}

\textbf{保存进程：}\cmd{autosave.c} 展示了 C 版的进程间通信。主服务端通过 pipe 向子进程发送 \cmd{SNAPSHOT}、\cmd{SAVE\_NOW}、\cmd{LOG} 和 \cmd{STOP} 事件。保存进程缓存最新快照，定期写入 \cmd{.autosave} 文件；手动保存会先生成 \cmd{.bak}，再用临时文件和 \cmd{rename()} 原子替换正式文件。

\inputminted[firstline=1,lastline=365,firstnumber=1]{c}{code/cowork/autosave.c}

\subsubsection{单元测试}

\textbf{测试代码：}\cmd{tests/test\_termsync.c} 使用简单断言宏组织测试。测试覆盖旧版本删除位置经过多次插入后右移、重复删除同一字符变为 \cmd{noop}、历史过期触发重同步、服务端版本严格递增、中文字符按 UTF-8 字符处理、客户端 ACK 队列、远程版本缺口、JSON Lines 转义，以及手动保存和自动保存文件生成。

\inputminted[firstline=1,lastline=358,firstnumber=1]{c}{code/cowork/tests/test_termsync.c}
