...
解决方案：客户端使用 C 的 UTF-8 与宽字符辅助函数，通过 \cmd{mbrtowc()}、\cmd{wcwidth()} 等能力计算宽字符显示列，并提供显示列和字符列之间的转换函数。测试中也包含中文字符插入、删除和长度计算场景。
...
TermSync 的 C 项目实现代码位于 \cmd{code/cowork} 目录。下面的结构按照客户端、服务端、文档模型、协议封装、持久化进程和测试代码进行划分：
