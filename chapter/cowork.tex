...
项目使用 C 语言和 POSIX 接口 \cmd{C 测试 runner} 进行自动化测试。实际运行时，可以在 Linux 服务器克隆 \cmd{code} 分支后，进入克隆得到的代码仓库根目录执行测试命令：
...
这些测试大多不依赖真实网络连接，而是直接验证核心状态变化。这样做可以使测试运行更快，也能让错误定位更集中。例如文档一致性问题可以直接在 \cmd{document.c} 的测试中暴露，不需要同时启动两个客户端和一个服务端才能复现。真实网络验证仍然是必要的，但它更适合放在后面的本地验证和部署验证中，用来确认多个模块组合后仍然能够正常工作。
...
本文所对应的项目代码托管于 GitHub 仓库 \url{https://github.com/glance02/Linux_Class.git} 中。其中，程序实现位于 \cmd{code} 分支，论文正文的 LaTeX 源码位于 \cmd{master} 分支。C 版本完整实现包含服务端、客户端、协议封装、持久化、日志和测试等多个模块，代码量较大；受篇幅限制，本节仍以关键代码片段为主。下文代码展示中涉及的文件路径，均以服务器克隆 \cmd{code} 分支后得到的代码仓库根目录为基准。

\subsubsection{项目结构}

TermSync 的 C 项目实现代码在 \cmd{code} 分支中位于仓库根目录。报告分支为了便于 \LaTeX{} 编译和代码引用，保留了同名代码文件副本；下面的项目结构按照服务器克隆 \cmd{code} 分支后的根目录书写：
...
\inputminted[firstline=1,lastline=60,firstnumber=1]{make}{code/cowork/Makefile}

\textbf{公共类型：}\cmd{termsync.h} 定义协议消息、编辑操作、传输层、文档状态、处理结果、客户端状态和保存进程等结构。C 语言没有动态字典和数据类，因此消息字段、字符串长度、内存所有权和释放规则都需要在公共头文件中明确约定。

\inputminted[firstline=1,lastline=120,firstnumber=1]{c}{code/cowork/termsync.h}
...
\inputminted[firstline=1,lastline=170,firstnumber=1]{c}{code/cowork/protocol.c}

\inputminted[firstline=260,lastline=430,firstnumber=260]{c}{code/cowork/protocol.c}
...
\inputminted[firstline=1,lastline=180,firstnumber=1]{c}{code/cowork/util.c}
...
\inputminted[firstline=1,lastline=130,firstnumber=1]{c}{code/cowork/document.c}
...
\inputminted[firstline=132,lastline=300,firstnumber=132]{c}{code/cowork/document.c}
...
\inputminted[firstline=1,lastline=120,firstnumber=1]{c}{code/cowork/events.c}
...
\inputminted[firstline=1,lastline=180,firstnumber=1]{c}{code/cowork/server.c}

\inputminted[firstline=181,lastline=360,firstnumber=181]{c}{code/cowork/server.c}
...
\inputminted[firstline=1,lastline=210,firstnumber=1]{c}{code/cowork/client_state.c}
...
\inputminted[firstline=1,lastline=180,firstnumber=1]{c}{code/cowork/client.c}

\inputminted[firstline=181,lastline=360,firstnumber=181]{c}{code/cowork/client.c}
...
\inputminted[firstline=1,lastline=170,firstnumber=1]{c}{code/cowork/autosave.c}

\inputminted[firstline=171,lastline=340,firstnumber=171]{c}{code/cowork/autosave.c}
...
\inputminted[firstline=1,lastline=220,firstnumber=1]{c}{code/cowork/tests/test_termsync.c}

\inputminted[firstline=221,lastline=360,firstnumber=221]{c}{code/cowork/tests/test_termsync.c}
