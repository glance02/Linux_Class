...
\begin{table}[H]
    \centering
    \caption{TermSync 核心模块职责划分}
    \small
    \begin{tabular}{p{0.18\textwidth} p{0.26\textwidth} p{0.25\textwidth} p{0.20\textwidth}}
        \toprule
        模块 & 主要职责 & 关键数据 & 课程知识点 \\
        \midrule
        \cmd{client.py} & 终端显示、键盘读取、光标移动、网络连接入口 & 屏幕行列、光标位置、用户按键 & 终端编程、网络客户端 \\
        \cmd{client\_state.py} & 维护客户端版本、待发送队列和未确认操作 & \cmd{pending\_queue}、\cmd{inflight\_op}、本地版本 & 状态机、队列思想 \\
        \cmd{protocol.py} & 将字典消息编码为 JSON Lines，并从 socket 字节流中恢复消息 & 字节缓冲区、JSON 对象、发送锁 & TCP 通信、消息边界 \\
        \cmd{document.py} & 保存权威文本、执行 OT、维护历史和版本 & 文本、版本号、历史操作、文档锁 & 线程同步、临界区 \\
        \cmd{server.py} & 接受连接、创建线程、分发消息、广播结果 & 客户端表、连接对象、保存队列 & socket 服务端、多线程 \\
        \cmd{autosave.py} & 写入自动保存文件、正式文件、备份和日志 & 快照、保存事件、日志事件 & 进程间通信、文件系统 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \caption{主要协议消息类型}
    \small
    \begin{tabular}{p{0.16\textwidth} p{0.20\textwidth} p{0.32\textwidth} p{0.20\textwidth}}
        \toprule
        消息类型 & 方向 & 主要字段 & 作用 \\
        \midrule
        \cmd{JOIN} & 客户端到服务端 & \cmd{client\_id} & 客户端加入协作文档，请求分配身份和初始状态 \\
        \cmd{WELCOME} & 服务端到客户端 & 确认后的 \cmd{client\_id} & 告诉客户端最终使用的编号，避免编号冲突 \\
        \cmd{DOC\_STATE} & 服务端到客户端 & 文本、版本号、历史起点 & 发送完整文档，用于新加入或重同步 \\
        \cmd{OP} & 客户端到服务端 & \cmd{op\_id}、\cmd{base\_version}、操作内容 & 提交字符级插入或删除操作 \\
        \cmd{ACK} & 服务端到提交者 & \cmd{op\_id}、服务端版本、权威操作 & 确认当前未完成操作，并驱动客户端发送队列继续前进 \\
        \cmd{REMOTE\_OP} & 服务端到其他客户端 & 服务端版本、权威操作 & 把已经确认的远程操作广播给其他客户端 \\
        \cmd{SAVE} & 客户端到服务端 & 客户端编号 & 请求手动保存当前权威文档 \\
        \cmd{RESYNC\_REQUIRED} & 服务端到客户端 & 当前版本和完整文档 & 客户端版本过旧时要求重新同步 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \caption{保存相关文件说明}
    \small
    \begin{tabular}{p{0.22\textwidth} p{0.24\textwidth} p{0.34\textwidth}}
        \toprule
        文件类型 & 生成时机 & 作用 \\
        \midrule
        正式文档 & 用户手动保存或服务端关闭前最终保存 & 保存当前确认的权威文档内容，是用户真正编辑的目标文件 \\
        \cmd{.autosave} & 服务端处理编辑后，保存进程按间隔写入 & 保存最近快照，降低异常退出造成内容丢失的风险 \\
        \cmd{.bak} & 覆盖正式文档前生成 & 保留上一次正式文件，避免手动保存误覆盖后无法回退 \\
        \cmd{.edit.log} & 加入、离开、操作、保存等事件发生时追加 & 记录服务端运行过程，便于调试和复现实验现象 \\
        临时文件 & 原子写入正式文件或自动保存文件时短暂存在 & 避免写入中断导致目标文件只剩半截内容 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.22\textwidth} p{0.30\textwidth} p{0.34\textwidth}}
        ...
        测试文件 & 覆盖对象 & 验证重点 \\
        \midrule
        \cmd{test\_document.py} & \cmd{CollaborativeDocument} 和 \cmd{Operation} & 服务端版本递增、OT 位置变换、重复删除、历史过期重同步、中文字符插入 \\
        \cmd{test\_client\_state.py} & 客户端状态机和显示辅助函数 & ACK 驱动队列、远程版本缺口、客户端编号确认、宽字符显示宽度、Backspace 操作排队 \\
        \cmd{test\_server.py} & 服务端事件输出和关闭逻辑 & \cmd{JOIN}、\cmd{OP}、\cmd{SAVE}、\cmd{LEAVE} 等输出格式，以及子进程关闭保护 \\
        \cmd{test\_autosave.py} & 保存与备份函数 & 手动保存前生成备份，并调用原子写入逻辑写入新内容 \\
        \bottomrule
    \end{tabular}
\end{table}
...
手动验证用于在不依赖公网、域名解析和安全组规则的情况下，先确认协作编辑器本身的核心功能是否正常。验证时在同一台 Linux 服务器上进行：一个终端运行服务端，同时使用 \cmd{tmux} 打开两个客户端窗口进行对比观察。首先在服务端终端中监听本机地址并指定协作编辑文件：
...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.24\textwidth} p{0.34\textwidth} p{0.28\textwidth}}
        ...
        验证场景 & 操作方法 & 预期结果 \\
        \midrule
        客户端加入 & 两个客户端分别连接 \cmd{127.0.0.1:8765} & 服务端输出两个 \cmd{JOIN}，两个客户端显示相同文档 \\
        英文输入 & 在一个客户端连续输入英文字符 & 另一个客户端同步出现相同内容，服务端版本递增 \\
        中文输入 & 输入中文字符并移动光标 & 终端显示列保持对齐，不出现明显错位 \\
        删除操作 & 使用 Backspace 删除字符 & 删除结果通过 ACK 和广播同步到两个客户端 \\
        手动保存 & 在客户端按下 \cmd{Ctrl-S} & 服务端输出保存请求，运行目录中出现保存产物 \\
        服务端关闭 & 终止服务端进程 & 客户端状态栏提示连接断开，程序不直接崩溃 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.22\textwidth} p{0.30\textwidth} p{0.34\textwidth}}
        ...
        环节 & 配置或现象 & 说明 \\
        \midrule
        监听地址 & 服务端监听 \cmd{0.0.0.0:8765} & 接受来自公网网卡的连接，而不只局限于本机回环地址 \\
        安全组 & 阿里云开放 \cmd{8765} 端口 & 云服务器防火墙允许外部客户端访问服务端进程 \\
        域名访问 & 客户端连接 \cmd{glance02.xyz} & 用户不需要记忆公网 IP，部署演示更接近真实服务 \\
        WSL 客户端 & Windows 电脑中使用 Linux 用户空间运行客户端 & 保证 \cmd{curses}、Python 和 TCP 编程环境可用 \\
        服务端日志 & 终端输出 \cmd{JOIN}、\cmd{OP}、\cmd{SAVE}、\cmd{LEAVE} & 可以从服务端侧确认连接、编辑、保存和退出流程 \\
        编辑日志 & \cmd{.edit.log} 中保存结构化事件 & 支持后续分析变换过程和保存行为 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.20\textwidth} p{0.30\textwidth} p{0.34\textwidth}}
        ...
        问题来源 & 可能后果 & 核心解决思路 \\
        \midrule
        多线程并发访问文档 & 文本状态不一致、版本号冲突 & 用文档锁保护权威状态，在临界区内分配全局版本 \\
        客户端版本落后 & 插入或删除位置失效 & 使用历史操作对旧版本操作执行 OT 变换 \\
        快速连续输入 & 未确认操作堆叠导致客户端版本混乱 & ACK 驱动发送队列，同一时刻只保留一个飞行中操作 \\
        慢网络或慢客户端 & 服务端锁被占用，其他客户端无法编辑 & 锁内只处理状态，锁外执行 socket 发送和广播 \\
        长时间运行 & 操作历史无限增长 & 限定历史长度，过旧客户端触发完整重同步 \\
        磁盘写入异常 & 正式文件被误覆盖或写坏 & 自动保存、备份文件和临时文件替换结合使用 \\
        终端宽字符 & 光标位置和文本索引错位 & 计算字符显示宽度，在字符列和显示列之间转换 \\
        \bottomrule
    \end{tabular}
\end{table}
...
TermSync 之所以适合作为 Linux 课程综合项目，是因为它不是只调用某一个库完成单点功能，而是把网络、线程、进程、文件、终端和测试这些知识点组合到同一个可运行系统中。项目中的每个模块都可以对应到课程中的一个主题，运行时也能观察到这些主题之间的协作关系。
