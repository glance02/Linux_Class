...
\begin{table}[H]
    \centering
    \caption{Shell 调试方法对比}
    \small
    \begin{tabular}{p{0.22\textwidth} p{0.30\textwidth} p{0.32\textwidth}}
        \toprule
        方法 & 适用阶段 & 作用 \\
        \midrule
        \cmd{bash -n linux\_quiz.sh} & 修改脚本结构后 & 只检查语法，不真正执行脚本，适合快速发现缺少 \cmd{fi}、\cmd{done} 等问题 \\
        \cmd{bash -x linux\_quiz.sh} & 逻辑结果不符合预期时 & 显示每条命令执行过程，便于观察变量值和分支走向 \\
        分段运行函数逻辑 & 定位具体功能时 & 把读取题库、判分、保存成绩等逻辑拆开观察 \\
        构造异常输入 & 功能基本完成后 & 验证文件缺失、非法菜单输入、空答案等情况是否会导致脚本崩溃 \\
        查看输出文件 & 保存功能调试时 & 检查 \cmd{score.txt} 是否按预期追加记录 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.22\textwidth} p{0.34\textwidth} p{0.30\textwidth}}
        ...
        扩展方向 & 实现思路 & 需要注意的问题 \\
        \midrule
        随机抽题 & 读取题库后使用随机排序或抽样 & 避免重复抽到同一道题，保持题目数量可控 \\
        错题记录 & 答错时把题目和用户答案追加到错题文件 & 注意错题文件格式，便于后续查看 \\
        排行榜 & 对历史成绩按分数排序并显示前几名 & 成绩记录格式要稳定，方便 \cmd{awk} 和 \cmd{sort} 处理 \\
        题库校验 & 启动时检查每行是否包含分隔符 & 避免格式错误导致读取题目和答案错位 \\
        多轮统计 & 记录总答题次数和平均分 & 需要从历史成绩中提取数值字段进行计算 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.20\textwidth} p{0.30\textwidth} p{0.34\textwidth}}
        ...
        变量 & 变化时机 & 作用 \\
        \midrule
        \cmd{score} & 答对题目后增加 & 记录当前轮得分 \\
        \cmd{answered} & 每完成一道题后增加 & 记录已经回答的题目数量 \\
        \cmd{wrong} & 答错题目后增加 & 控制错误次数上限 \\
        \cmd{question} & 每读取一行题库后更新 & 保存当前题目文本 \\
        \cmd{correct\_answer} & 每读取一行题库后更新 & 保存当前题目的标准答案 \\
        \cmd{user\_answer} & 用户输入后更新 & 保存用户本次提交的答案 \\
        \bottomrule
    \end{tabular}
\end{table}
